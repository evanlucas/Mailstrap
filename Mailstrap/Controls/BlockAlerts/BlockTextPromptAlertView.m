...
- (id)initWithTitle:(NSString *)title message:(NSString *)message defaultText:(NSString*)defaultText block: (TextFieldReturnCallBack) block {
    ...
    self = [super initWithTitle:title message:message usingInterfaceOrientation:[AppDelegate orientation]];
    
    if (self) {
        LargeTextField *theTextField = [[[LargeTextField alloc] initWithFrame:CGRectMake(kTextBoxHorizontalMargin, _height, _view.bounds.size.width - kTextBoxHorizontalMargin * 2, kTextBoxHeight)] autorelease];
