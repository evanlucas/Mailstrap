...
+ (void)initialize
{
    if (self == [BlockAlertView class])
    {
        ...
        background = [BlockAlertView bgImage];
        background = [[background resizableImageWithCapInsets:UIEdgeInsetsZero] retain];
...
+ (UIImage *)bgImage {
    UIGraphicsBeginImageContext(CGSizeMake(280, 100));
    CGContextRef ctx = UIGraphicsGetCurrentContext();
    UIColor *mscolor = [UIColor colorWithWhite:0.1 alpha:1.0];
    CGContextSetFillColorWithColor(ctx, mscolor.CGColor);
    CGContextFillRect(ctx, CGRectMake(0, 0, 280, 100));
...
{
    return [[[BlockAlertView alloc] initWithTitle:title message:message usingInterfaceOrientation:[AppDelegate orientation]] autorelease];
}
...
- (void)show
{
    ...
    for (NSUInteger i = 0; i < _blocks.count; i++)
    {
        ...
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"alert-%@", color]];
        ...
        else if (_blocks.count  == 1)
        {
            ...
            // I want the fucking button to be as wide as the label
            //
            ...
            width = _view.bounds.size.width-kAlertViewBorder*2;
            xOffset = kAlertViewBorder;
        }
