...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  NSNumber *delivered = [campaign delivered_count];
  switch (indexPath.row) {
    ...
    case 4:
      ...
      cell.detailTextLabel.text = [self valueWithNumber:campaign.clicked_count delivered:delivered];
      break;
    case 5:
      cell.textLabel.text = @"Opened";
      cell.detailTextLabel.text = [self valueWithNumber:campaign.opened_count delivered:delivered];
      break;
    case 6:
    ...
    case 7:
      ...
      cell.detailTextLabel.text = [self valueWithNumber:campaign.bounced_count delivered:delivered];
      break;
    case 8:
      cell.textLabel.text = @"Complained";
      cell.detailTextLabel.text = [self valueWithNumber:campaign.complained_count delivered:delivered];
      break;
    case 9:
      cell.textLabel.text = @"Unsubscribed";
      cell.detailTextLabel.text = [self valueWithNumber:campaign.unsubscribed_count delivered:delivered];
      break;
    case 10:
      cell.textLabel.text = @"Dropped";
      cell.detailTextLabel.text = [self valueWithNumber:campaign.dropped_count delivered:delivered];
      break;
    default:
      break;
  }
...
- (NSString *)valueWithNumber:(NSNumber *)c delivered:(NSNumber *)delivered {
  // C is current count
  // delivered is the number delivered
  // percent is c / submitted
  if ([delivered isEqual: @0]) {
    return [NSString stringWithFormat:@"%@ - %@ / %@", @"0%", @0, @0];
  }
  NSString *cs = [NSString stringWithFormat:@"%@", c];
  NSString *ss = [NSString stringWithFormat:@"%@", delivered];
  ...
  return [NSString stringWithFormat:@"%@ - %@ / %@", [nf stringFromNumber:d], c, delivered];
}

- (NSString *)valueWithNumber:(NSNumber *)c submitted:(NSNumber *)submitted {
  if ([submitted isEqual:@0]) {
    return [NSString stringWithFormat:@"%@ - %@ / %@", @"0%", @0, @0];
  }
  NSString *cs = [NSString stringWithFormat:@"%@", c];
