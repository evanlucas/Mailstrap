...
- (void)viewDidLoad {
  ...
  [self.switchOrderResults setOn:[[APIController sharedInstance] shouldOrderResults]];
...
- (IBAction)tappedSaveAction:(id)sender {
  ...
  [[APIController sharedInstance] setShouldOrderResults:self.switchOrderResults.isOn];
...
- (void)viewDidUnload {
  [self setSwitchOrderResults:nil];
  [super viewDidUnload];
}
@end
